...
def run(config: EvalConfig):
    ...
    with open(config.metrics_save_path / f"blip_raw_{file_name}_metrics.json", 'w') as f:
        json.dump(results_per_prompt, f, sort_keys=True, indent=4)
    with open(config.metrics_save_path / f"blip_aggregated_{file_name}_metrics.json", 'w') as f:
