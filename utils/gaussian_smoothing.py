...
# -----------------------------------------------------------------------------
#   Gaussian smoothing (patched for torch>=2.1)
# -----------------------------------------------------------------------------
class GaussianSmoothingPatch(torch.nn.Module):
    def __init__(
        self,
        channels: int = 1,
        ...
        # --- two‑step view + repeat (fixes dim mismatch on torch≥2.1) --------
        kernel = kernel.view(1, 1, *kernel.size())
        kernel = kernel.repeat(channels, *[1] * (kernel.dim() - 1))
    ...
    def forward(
        self, x: torch.Tensor
    ) -> torch.Tensor:  # noqa: D401 – simple pass‑through
